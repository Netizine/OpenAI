namespace OpenAI.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// At the moment this test only checks `public` properties. `internal` properties are ignored.
    /// </remarks>
        ...
        public void Check()
        {
            ...
            foreach (Type openAIClass in openAIClasses)
            {
                foreach (PropertyInfo property in openAIClass.GetProperties())
                {
                    ...
                    results.Add($"{openAIClass.Name}.{property.Name}: {property.Name} != {pascalCasedJsonName}");
                }
            }
